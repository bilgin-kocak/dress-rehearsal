...
|---|---|---|
| 0:00–0:15 | `prompts/strategy_momentum_v1.md` open in the editor (10 lines). Cut to the Binance docs line "no paper mode" and the terminal: `claude mcp add binance-mcp-server … agent.binance.com`. | "This is a first-draft momentum strategy for a Binance Agent OS agent. The MCP it will trade through has no paper mode. Before you give it capital, give it a dress rehearsal." |
...
| 2:00–2:10 | Closing card: "Same agent. Same prompt. Change one URL." + repo URL + `evidence/README.md`. | "Dress Rehearsal. Find the mistake, understand it, retest the fix, then go live. The evidence bundle is in the repo." |
