...
Procedure:
1. spot.getAccount (omitZeroBalances=true) → free USDT. futures_usds.markPrice(symbol=BTCUSDT) → current funding rate (lastFundingRate) and mark price.
2. If lastFundingRate is positive (longs pay shorts) continue; otherwise report "funding not favourable" and stop.
3. Read spot.exchangeInfo(symbol=BTCUSDT) and futures_usds.exchangeInfo once. Record stepSize / tickSize / minNotional for both markets. The futures BTCUSDT minimum notional is 100 USDT.
...
5. futures_usds.changeLeverage(symbol=BTCUSDT, leverage=2). Restate before calling.
...
8. Verify with futures_usds.positionRisk(symbol=BTCUSDT) and spot.getAccount that the legs match (long spot ≈ short futures).
